...
class UserAchievementAdapter :
 RecyclerView.Adapter<UserAchievementAdapter
 ...
 override fun onCreateViewHolder(parent: ViewGroup,
  ...
  return UserAchievementViewHolder(
   LayoutInflater.from(parent.context).inflate(
    R.layout.user_achievement_card,
    parent,
    false
   )
  )
 ...
 override fun onBindViewHolder(holder:
  ...
  holder.userDistance.text =
   "Totale Amount of achievements: " +
   "${users[position].completedAchievements.count()}"
  Picasso.with(context).load(users[position].imageUrl)
   .error(R.drawable.ic_user_dark)
   .into(holder.userImage)
 }

 inner class UserAchievementViewHolder(itemview: View) :
