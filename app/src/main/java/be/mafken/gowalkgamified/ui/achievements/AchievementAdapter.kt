...
class AchievementAdapter : RecyclerView.Adapter<AchievementAdapter.AchievementViewHolder>() {
    ...
    var achievements: List<Achievement> by Delegates.observable(emptyList()) { _, _, _ -> notifyDataSetChanged() }
    var user: User by Delegates.observable(User()) { _, _, _ -> notifyDataSetChanged() }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): AchievementViewHolder {
        return AchievementViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.achievement_card, parent, false))
    }
    ...
    override fun onBindViewHolder(holder: AchievementViewHolder, position: Int) {
        ...
        holder.achievementDescription.text = achievements[position].description

        holder.achievementCompleted.isChecked = user.completedAchievements.contains(achievements[position].id)
    }
